	...
	for(int i=0; i<Files.Get_Count(); i++)
	{
		...
		try {
			while( reader.ReadNextPoint() )
			{
				...
				LAS_POINT const& point = reader.GetPoint();
				...
				pPoints->Add_Point(point.GetX(), point.GetY(), point.GetZ());
                                ...
                                LAS_UINT8 cls = point.GetClassification().GetClass();
#else
                                LAS_UINT8 cls = p.GetClassification();
#endif
				...
				if( iField[VAR_c] > 0 )	pPoints->Set_Value(iPoint, iField[VAR_c], cls);
