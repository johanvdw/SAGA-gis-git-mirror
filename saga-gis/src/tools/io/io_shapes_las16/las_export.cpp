...
bool CLAS_Export::On_Execute(void)
{
	...
	LAS_UINT32	binPulse[MAX_NUM_RETURN];
	LAS_UINT16	r;
	...
	LAS_HEADER header;
        ...
        switch (format)
	{
	case 0:		header.SetDataFormatId(LAS_EPOINTFORMAT0);	break;
	case 1:		header.SetDataFormatId(LAS_EPOINTFORMAT1);	break;
	case 2:		header.SetDataFormatId(LAS_EPOINTFORMAT2);	break;
	case 3:
	default:	header.SetDataFormatId(LAS_EPOINTFORMAT3);	break;
	}
	...
	for( int i=0; i<pPoints->Get_Count(); i++ )
	{
		...
		if( i_i > -1 )
			point.SetIntensity((LAS_UINT16)pPoints->Get_Value(i, i_i));
		if( i_a > -1 )
			point.SetScanAngleRank((LAS_INT8)pPoints->Get_Value(i, i_a));
		if( i_r > -1 )
		{
			r = (LAS_UINT16)pPoints->Get_Value(i, i_r);
		...
		if( i_n > -1 )
			point.SetNumberOfReturns((LAS_UINT16)pPoints->Get_Value(i, i_n));
		if( i_c > -1 )
			point.SetClassification((LAS_INT8)pPoints->Get_Value(i, i_c));
		if( i_u > -1 )
			point.SetUserData((LAS_INT8)pPoints->Get_Value(i, i_u));
		if( i_R > -1 && i_G > -1 && i_B > -1 )
		{
			LAS_COLOR	color;
			color.SetRed((LAS_UINT16)pPoints->Get_Value(i, i_R));
			color.SetGreen((LAS_UINT16)pPoints->Get_Value(i, i_G));
			color.SetBlue((LAS_UINT16)pPoints->Get_Value(i, i_B));
			point.SetColor(color);
		}
		else if( i_RGB > -1 )
		{
			LAS_COLOR	color;
			color.SetRed((LAS_UINT16)SG_GET_R((int)pPoints->Get_Value(i, i_RGB)));
			color.SetGreen((LAS_UINT16)SG_GET_G((int)pPoints->Get_Value(i, i_RGB)));
			color.SetBlue((LAS_UINT16)SG_GET_B((int)pPoints->Get_Value(i, i_RGB)));
			point.SetColor(color);
		}
		if( i_e > -1 )
			point.SetFlightLineEdge((LAS_UINT16)pPoints->Get_Value(i, i_e));
		if( i_d > -1 )
			point.SetScanDirection((LAS_UINT16)pPoints->Get_Value(i, i_d));
		if( i_p > -1 )
			point.SetPointSourceID((LAS_UINT16)pPoints->Get_Value(i, i_p));
